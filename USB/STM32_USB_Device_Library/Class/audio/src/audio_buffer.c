    ...
    switch(GET_CONFIG_CA_ALGORITM(handle->um_buffer_config))
    {
        case UM_BUFFER_CONFIG_CA_NONE:
            if(handle->cur_um_node_for_usb->um_node_offset == 0)
            {
                ...
                && handle->cur_um_node_for_usb->um_node_state != UM_NODE_STATE_INITIAL)
                {
                    /* Buffer overflow */
                    return result;
                }
...
void audio_dma_complete_cb(struct um_buffer_handle *handle)
{
    ...
    && handle->cur_um_node_for_hw->um_node_state != UM_NODE_STATE_INITIAL)
    {
        /* State machine error */
        while(1) {}
    }
    ...
    case UM_NODE_STATE_UNDER_USB:
        if(handle->cur_um_node_for_hw != handle->cur_um_node_for_usb)
        {
            ...
            In that case cur_um_node_for_hw should be equal to cur_um_node_for_usb.
